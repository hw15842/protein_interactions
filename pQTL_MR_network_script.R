...
extract_from_VCF <- function(exposure_protein_number, exposure_protein_name, chr, rsid_position, SNP){
...
## Test ##
#nrow(new_table)
#new_table2 <- new_table[c(270:300),]       #1622
#results_for_coloc_test <- mapply(extract_from_VCF, new_table2$protein_number, new_table2$Platform_id, new_table2$CHR_SNP, new_table2$POS_SNP)
#results_for_coloc_test_table <- data.table(t(results_for_coloc_test))
#head(results_for_coloc_test_table)
#pastefile_test <- paste0(results_location, "/prot-a-", outcome_protein, "_coloc_results_test.rdata")
#save(results_for_coloc_test_table, file=pastefile_test)



results_for_coloc_full <- mapply(extract_from_VCF, new_table$protein_number, new_table$Platform_id, new_table$CHR_SNP, new_table$POS_SNP, new_table$SNP)
