#######################################
#### new_results_Folkerson_script.R ###
#######################################
...
zheng_pQTLS_formated <- format_data(Zheng_pQTLs_with_file_names, type="exposure", phenotype_col = "Platform_id", snp_col ="SNP", beta_col="Beta", se_col ="SE", 
...
pastename1_outcome_protein_location <- paste0(BC4_data_location, "prot-b-", outcome_protein, "/", "prot-b-", outcome_protein, ".vcf.gz") # this is becuase need to go into the protein file and then get the vcf file 
...
Folkerson_results_protein_interactions_MR <- lapply(zheng_pQTLs_individual_proteins, run_protein_MR)
Folkerson_results_protein_interactions_MR_table <- ldply(Folkerson_results_protein_interactions_MR, data.table)

### Save the results 

pastefile1 <- paste0("Folkerson_prot-b-", outcome_protein, "_MR_results.rdata")
save(Folkerson_results_protein_interactions_MR_table, file=pastefile1)
...
### Need to be able to read the Sun and Folkerson full sum stats (not doing Suhre anymore)

## Split the zheng pqtl file into sun, folkerson and suhre 
zheng_pQTLs_SUN_only <- Zheng_pQTLs_with_file_names[grepl("prot-a", Zheng_pQTLs_with_file_names$prot_file_name),]
zheng_pQTLs_FOLKERSON_only <- Zheng_pQTLs_with_file_names[grepl("prot-b", Zheng_pQTLs_with_file_names$prot_file_name),]

### then will have to run all three seperately as are in different locations 


##### Coloc of pQTLs from SUN proteins on Folkerson Proteins (which are being iterated through via the .sh file)

Coloc_SUN_exposure <- function(Sun_protein_file_name, chr, rsid_position, SNP, Platform_id){
  bp_start <- (rsid_position - 500000)
  bp_finish <- (rsid_position + 500000)
  bp_start[bp_start<0] <- 1
  Sun_protein_VCF_location <- paste0(BC4_data_location, Sun_protein_file_name, "/", Sun_protein_file_name, ".vcf.gz")
  chr_position <- paste0(chr, ":", bp_start,"-", bp_finish)
  vcfsubset1 <- tryCatch(query_gwas(Sun_protein_VCF_location, chrompos=c(chr_position)), error=function(e) NA)  ## VCF subset 1 is the Sun proteins
  vcfsubset2 <- tryCatch(query_gwas(pastename1_outcome_protein_location, chrompos=c(chr_position)), error=function(e) NA) ### VCF subset 2 is the Folkerson protein that the .sh file is iterating through
  ...
  coloc_results <- tryCatch(data.frame(coloc_results), error=function(e) NA)
  coloc_results$protein_number = as.character(Sun_protein_file_name)
  coloc_results$platform_id = as.character(Platform_id)
  coloc_results$outcome_protein_number = as.character(paste0("prot-b-", outcome_protein))
  coloc_results$rsid = as.character(SNP)
  return(coloc_results)
}

Sun_on_Folkerson_coloc_results <- mapply(Coloc_SUN_exposure, zheng_pQTLs_SUN_only$prot_file_name, zheng_pQTLs_SUN_only$CHR_SNP, zheng_pQTLs_SUN_only$POS_SNP, zheng_pQTLs_SUN_only$SNP, zheng_pQTLs_SUN_only$Platform_id)

Sun_on_Folkerson_coloc_results_table <- ldply(Sun_on_Folkerson_coloc_results, data.table)



pastefile2 <- paste0("Sun_on_Folkerson_prot-b-", outcome_protein, "_coloc_results.rdata")
save(Sun_on_Folkerson_coloc_results_table, file=pastefile2)





##### Coloc of pQTLs from FOLKERSON proteins on FOLKERSON Proteins (which are being iterated through via the .sh file)
...
Coloc_FOLKERSON_exposure <- function(Folkerson_protein_file_name, chr, rsid_position, SNP, Platform_id){
  ...
  coloc_results <- tryCatch(t(coloc_results), error=function(e) NA)
  coloc_results <- tryCatch(data.frame(coloc_results), error=function(e) NA)
  coloc_results$protein_number = as.character(Folkerson_protein_file_name)
  coloc_results$platform_id = as.character(Platform_id)
  coloc_results$outcome_protein_number = as.character(paste0("prot-b-", outcome_protein))
  coloc_results$rsid = as.character(SNP)
  return(coloc_results)
...
Folkerson_on_Folkerson_coloc_results <- mapply(Coloc_FOLKERSON_exposure, zheng_pQTLs_FOLKERSON_only$prot_file_name, zheng_pQTLs_FOLKERSON_only$CHR_SNP, zheng_pQTLs_FOLKERSON_only$POS_SNP, zheng_pQTLs_FOLKERSON_only$SNP, zheng_pQTLs_FOLKERSON_only$Platform_id)
Folkerson_on_Folkerson_coloc_results_table <- data.table(t(Folkerson_on_Folkerson_coloc_results))

head(Folkerson_on_Folkerson_coloc_results_table)

pastefile3 <- paste0("Folkerson_on_Folkerson_prot-b-", outcome_protein, "_coloc_results.rdata")
save(Folkerson_on_Folkerson_coloc_results_table, file=pastefile3)
